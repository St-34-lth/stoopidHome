...
if __name__ == "__main__":
    ...
    package_path = rospack.get_path("data_sink")

    # Build absolute paths to the certificate files under the "ss/" folder
    ca_certs = os.path.join(package_path, "ssl", "ca.crt")         # CA certificate file
    certfile = os.path.join(package_path, "ssl", "client.crt")       # (optional) client certificate
    keyfile = os.path.join(package_path, "ssl", "client.pem")
    mqtt_client.tls_set(
    ca_certs,           # CA certificate file
    certfile,       # (optional) client certificate
    keyfile,        # (optional) client key
